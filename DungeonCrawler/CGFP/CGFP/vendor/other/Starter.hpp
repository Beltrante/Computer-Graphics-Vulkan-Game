...
//		createInfo.ppEnabledExtensionNames = glfwExtensions;
	...
	void getSixAxis(float &deltaT, glm::vec3 &m, glm::vec3 &r, bool &fire, bool &click) {
		...
		click = glfwGetKey(window, GLFW_KEY_ENTER) | glfwGetMouseButton(window, GLFW_MOUSE_BUTTON_LEFT) == GLFW_PRESS;
...
void Model<Vert>::loadModelGLTF(std::string file, bool encoded) {
    ...
    // save encrypted model from MGCG to GLTF added by Matteo
	if(encoded){
		// Extract the file name and path
		size_t lastDot = file.find_last_of(".");
		std::string baseFileName = file.substr(0, lastDot);
		// Save the decrypted model with the same path and name as the input file but with ".gltf" extension
    	std::string outputFile = baseFileName + ".gltf";

		std::cout << "SAVING FILE: "<< baseFileName << "\n";	
		if (!loader.WriteGltfSceneToFile(&model, outputFile, true, true, true, false)) {
			std::cout << "ERROR SAVING FILE: "<< baseFileName << "\n";	
			throw std::runtime_error("Error writing GLTF file");
		}
	}
